...
import io.gravitee.resource.ai.vector.store.api.VectorStore;
import io.gravitee.resource.ai_model.api.AiTextModelResource;
import io.gravitee.resource.ai_model.api.result.EmbeddingResult;
...
public record ResourceProvider(AiRagPolicyConfiguration configuration) {
  ...
  public VectorStore vectorStore(HttpPlainExecutionContext ctx) {
    return ctx
      .getComponent(ResourceManager.class)
      .getResource(
        configuration.vectorStoreName(),
        AiVectorStoreResource.class
      );
  }
}
